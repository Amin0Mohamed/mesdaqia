...
class AcceptOrderController extends Controller
{
    ...
    protected function Reject_order($id,$ownerID,$category)
    {
        if($category=='cars'){
            $notification = notifications::create(
                [
                    ...
                    'ownerID' =>$ownerID,
                    'category'=>$category,
                    'Message'=>'من فضلك ادخل مزاد سيارات صحيح',
                ]
            );
            session()->flash('success',__('site.send_notification_to_user_refuse_order'));
            return back();
        }elseif ($category=='jewelries'){
            $notification = notifications::create(
                [
                    'ProductID'=>$id,
                    'ownerID' =>$ownerID,
                    'category'=>$category,
                    'Message'=>'من فضلك ادخل مزاد مجوهرات صحيح',
                ]
            );
            session()->flash('success',__('site.send_notification_to_user_refuse_order'));
            return back();
        }elseif ($category=='highvalues'){
            $notification = notifications::create(
                [
                    'ProductID'=>$id,
                    'ownerID' =>$ownerID,
                    'category'=>$category,
                    'Message'=>'من فضلك ادخل مزاد اخري ثمينه صحيح',
                ]
            );
            session()->flash('success',__('site.send_notification_to_user_refuse_order'));
            return back();

        }elseif ($category=='vichles'){
            $notification = notifications::create(
                [
                    'ProductID'=>$id,
                    'ownerID' =>$ownerID,
                    'category'=>$category,
                    'Message'=>'من فضلك ادخل مزاد اخري مركبات صحيح',
                ]
            );
            session()->flash('success',__('site.send_notification_to_user_refuse_order'));
            return back();
            ...
            $notification = notifications::create(
                [
                    ...
                    'ownerID' =>$ownerID,
                    'category'=>$category,
                    'Message'=>'من فضلك ادخل مزاد اخري عقارات صحيح',
                ]
            );
            session()->flash('success',__('site.send_notification_to_user_refuse_order'));
            return back();
